...
{
    private readonly int multiplicator;
    private readonly StatType stat;

    public PercentualDamageReductionDeterminedByStatDifferenceEffect(StatType stat, int multiplicator)
    {
        this.stat = stat;
    ...
    public override void ApplyEffect(Unit myUnit, Unit opponentsUnit)
    {
        ...
        if (stat == StatType.Spd)
        {
            ...
            redutionPercentage = 1 - (myTotalSpd - opponentsTotalSpd) * multiplicator / 100;
        }
        else if (stat == StatType.Res)
        {
            ...
            redutionPercentage = 1 - (myTotalRes - opponentsTotalRes) * multiplicator / 100;
        }

        if (redutionPercentage < 0.6) redutionPercentage = 0.6;
